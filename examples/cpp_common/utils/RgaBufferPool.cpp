...
struct ExternalBufferGroup {
  ...
  ExternalBufferGroup(RK_U32 count, RK_U32 size)
      : buf_count(count), buf_size(size), group(nullptr), buf_list(nullptr) {
    MPP_RET ret = MPP_NOK;
...
ExtMppBufferAllocator::ExtMppBufferAllocator(int count, uint32_t size)
    : buf_count(count), buf_size(size), buffer_group_(new ExternalBufferGroup(count, size)) {}
...
RgaBufferPool::RgaBufferPool(size_t pool_size, RgaSURF_FORMAT format, int w, int h,
                             BufferType buf_type)
...
bool RgaBufferPool::ImportRgaBuffer(rga_buffer_t* buffer) {
  ...
  // create dma buffer
  rga_buffer_size_ = (size_t) get_bpp_from_format(format_) * MPP_ALIGN(width_, 16) * height_;
  ...
  if (buf_type_ == kBufferTypeCpu) {
    ...
    ret = dma_buf_alloc(DMA_HEAP_DMA32_UNCACHED_PATH, rga_buffer_size_, &fd, (void**) &vir_addr);
    if (ret != 0) {
  ...
  // save the buffer info
  *buffer = wrapbuffer_handle(rga_handle, width_, height_, (int) handle_param.format, hor_stride_,
                              ver_stride_);
  buffer->fd = fd;
