...
int MppEncoder::InitParams(MppEncoderParams& params) {
  ...
  switch (enc_params_.fmt & MPP_FRAME_FMT_MASK) {
    ...
    case MPP_FMT_RGBA8888: {
      frame_size_ = MPP_ALIGN(enc_params_.hor_stride, 64) * MPP_ALIGN(enc_params_.ver_stride, 64);
    } break;
  ...
  if (MPP_FRAME_FMT_IS_FBC(enc_params_.fmt)) {
    if ((enc_params_.fmt & MPP_FRAME_FBC_MASK) == MPP_FRAME_FBC_AFBC_V1)
      header_size_ = MPP_ALIGN(
          MPP_ALIGN(enc_params_.width, 16) * MPP_ALIGN(enc_params_.height, 16) / 16, SZ_4K);
    else
...
int MppEncoder::SetupEncCfg() {
  ...
  switch (enc_params_.rc_mode) {
    ...
    case MPP_ENC_RC_MODE_CBR: {
      ...
      mpp_enc_cfg_set_s32(cfg_, "rc:bps_max",
                          enc_params_.bps_max ? enc_params_.bps_max : enc_params_.bps * 17 / 16);
      mpp_enc_cfg_set_s32(cfg_, "rc:bps_min",
  ...
  if (enc_params_.split_mode) {
    LOGD(TAG, "%p split mode %d arg %d out %d", mpp_ctx_, enc_params_.split_mode,
         enc_params_.split_arg, enc_params_.split_out);
...
int MppEncoder::Init(MppEncoderParams& params) {
  ...
  LOGD(TAG, "mpp encoder started: w %d h %d type %d", enc_params_.width, enc_params_.height,
       enc_params_.type);
...
MPP_RET MppEncoder::Encode(const rga_buffer_t* rga_buffer) {
  if (rga_buffer->fd > 0) {
    ...
    if (rga_buffer->width != enc_params_.width || rga_buffer->height != enc_params_.height) {
      LOGE(TAG, "input frame size %dx%d is not equal to encoder size %dx%d", rga_buffer->width,
           rga_buffer->height, enc_params_.width, enc_params_.height);
      return MPP_NOK;
    }
...
size_t MppEncoder::Encode(const VideoFrameSlot& f, char** encoded_data) {
  ...
  if (f.fd > 0) {
    ...
    if (encoded_video_info_.pixel_format == 0) {
      // YUV420SP
      info.size = f.width_stride * f.height_stride * 3 / 2;
    } else if (encoded_video_info_.pixel_format == 1) {
      // MPP_FMT_RGBA8888
      info.size = f.width_stride * f.height_stride * 4;
    } else {
      info.size = f.width_stride * f.height_stride * 3;
